    ...
    def _execute_handlers(skip_conditions, handlers, context, phase):
        ...
        for handler in handlers:
            ...
            if data:
                # FIXME
                context.update(data)
            # In case of OAuth flow after refreshing access token need to try again with the query to collect records
...
class CCEHTTPRequestTask(BaseTask):
    ...
    def _send_request(self, request):
        ...
        # This is special handling in case of SOQL query to salesforce object fails due unsupported sorting order 'ASCENDING'
        # Hence it means salesforce object supports only 'DESCENDING' order
        if status in (400,):
        ...
        # This is special handling in case of autherization error we want to continue with execution

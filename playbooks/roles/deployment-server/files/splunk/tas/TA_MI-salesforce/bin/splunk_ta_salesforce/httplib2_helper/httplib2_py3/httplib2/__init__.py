...
# Temporarily removed dependency on any compatible module called socks until httplib2 supports it.
# It will use socks.py within its package.
try:
...
"""
    ...
    def _request(
        ...
        # Here we have added special handling for www-authenticate header with token value. As salesforce unauthorized
        # response doesn't contain valid www-authenticate and it is raising malformed header error. So we have added
        # explicit condition to avoid this error.
        if response.status == 401 and response['www-authenticate'] != 'Token':
